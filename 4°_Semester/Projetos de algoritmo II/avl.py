...
#a e b sao as alturas de dois nos da arvore
def maior(a, b):
    return a if a > b else b

#retorna a altura do no ou -1 caso ele nao exista
def altura_do_no(no):
    if no is None:
        return -1
    return no.altura
...
def rotacao_esquerda(raiz):
    ...
    #salva o no a direita da raiz
    e = raiz.direita

    #salva o possivel filho do no a direita
    filho = e.esquerda

    ##apos que as os valores dos nos foram salvos, é possível começar as alterações

    #o no a esquerda do y agora aponta para a raiz
    e.esquerda = raiz
    raiz.direita = filho
    raiz.altura = maior(altura_do_no(raiz.esquerda), altura_do_no(raiz.direita)) + 1
    e.altura = maior(altura_do_no(e.esquerda), altura_do_no(e.direita)) + 1
    return e

def rotacao_direita(raiz):

    #salva o no a esquerda da raiz
    d = raiz.esquerda

    #salva o possivel filho do no a esquerda
    filho = d.direita

     ##apos que as os valores dos nos foram salvos, é possível começar as alterações

     #o no a direita do y agora aponta para a raiz
    d.direita = raiz
    raiz.esquerda = filho
    raiz.altura = maior(altura_do_no(raiz.esquerda), altura_do_no(raiz.direita)) + 1
    d.altura = maior(altura_do_no(d.esquerda), altura_do_no(d.direita)) + 1
    return d
